...
if [[ -z "$CRDB_ORG_NAME" ]]; then
    echo "Must provide CRDB_ORG_NAME as the first parameter. Example './up.sh CRDB_ORG_NAME CRDB_LICENSE_KEY'." 1>&2
    exit 1
fi

if [[ -z "$CRDB_LICENSE_KEY" ]]; then
    echo "Must provide CRDB_LICENSE_KEY as the second parameter. Example './up.sh CRDB_ORG_NAME CRDB_LICENSE_KEY'." 1>&2
    exit 1
fi
